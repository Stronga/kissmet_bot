...
export function formatMinorUnits(amountMinor: number | null | undefined, currency = "GHS"): string {
  if (amountMinor === null || amountMinor === undefined || Number.isNaN(amountMinor)) return "Unavailable";
...
export function percentageToBasisPoints(percentInput: string): number {
  ...
  if (!/^\d+(\.\d{1,2})?$/.test(trimmed)) throw new Error("Percentage must be a number such as 50 or 50.00");
  const [majorRaw, fracRaw = ""] = trimmed.split(".");
  const major = Number.parseInt(majorRaw, 10);
  const frac = Number.parseInt((fracRaw + "00").slice(0, 2), 10);
  return major * 100 + frac;
